...
class WorkspaceApi {
  ...
  async listAgents(): Promise<WorkspaceAgent[]> {
    ...
    return discovery.agents.map((a) => ({
      ...
      presenceStatus: a.presence_status || a.status,
      activityState: a.activity_state || a.lifecycle_state || a.status,
      workloadState: a.workload_state || null,
      displayStatus: a.display_status || a.status,
      isConnected: a.is_connected ?? a.status === 'online',
      hasActiveWork: a.has_active_work ?? false,
