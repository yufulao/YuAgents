...
export function AgentStatusCard({ agents }: AgentStatusCardProps) {
  ...
  const statusText = (agent: WorkspaceAgent) => {
    if (agent.activityState && agent.activityState !== 'idle') return agent.activityState;
    if (agent.presenceStatus || agent.status) return agent.presenceStatus || agent.status;
    return agent.lastHeartbeatAt ? `offline · ${timeAgo(agent.lastHeartbeatAt)}` : 'offline';
  };
